...
export function BlogListingClient({ posts }: { posts: BlogPost[] }) {
  ...
  return (
    <div>
      <div className="flex flex-col gap-4 rounded-2xl border border-border bg-white p-4 shadow-soft lg:flex-row lg:items-center lg:justify-between">
        ...
        <label className="relative block w-full lg:w-72"><Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-text-subtle" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search articles" className="w-full rounded-xl border border-border bg-white py-2.5 pl-10 pr-9 text-sm text-text outline-none transition-shadow placeholder:text-text-subtle focus:border-primary focus:ring-4 focus:ring-primary/10" aria-label="Search blog articles" />{query && <button type="button" onClick={() => setQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-subtle hover:text-text" aria-label="Clear search"><X className="h-4 w-4" /></button>}</label>
      </div>
      ...
      {featured ? <>
        <section className="mt-10" aria-labelledby="featured-article-title">
          ...
          <Link href={`/blog/${featured.slug}`} className="group grid overflow-hidden rounded-3xl border border-border bg-white shadow-soft transition-all duration-300 hover:-translate-y-1 hover:border-primary/20 hover:shadow-soft-lg md:grid-cols-[1.12fr_1fr]">
        ...
        <section className="mt-12" aria-labelledby="latest-articles-title"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Keep learning</p><h2 id="latest-articles-title" className="mt-2 text-2xl font-semibold tracking-tight text-text">Latest articles</h2></div><p className="hidden text-sm text-text-muted sm:block">Practical guides for real media workflows.</p></div>{latest.length > 0 ? <div className="mt-6 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{latest.map((post) => <BlogCard key={post.slug} post={post} />)}</div> : <div className="mt-6 rounded-2xl border border-dashed border-border bg-white p-8 text-center text-sm text-text-muted">This is the only article matching the current filter.</div>}</section>

        <section className="mt-14 rounded-3xl border border-border bg-gradient-brand-soft p-7 md:p-10" aria-labelledby="topic-discovery-title"><div className="max-w-2xl"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Explore by topic</p><h2 id="topic-discovery-title" className="mt-3 text-2xl font-semibold tracking-tight text-text">Start with the platform or workflow that matters now.</h2><p className="mt-3 text-sm leading-6 text-text-muted">Browse the categories already used across SavDownâ€™s editorial library.</p></div><div className="mt-7 flex flex-wrap gap-3">{categories.filter((item) => item !== 'All').map((item) => <button key={item} type="button" onClick={() => setCategory(item)} className="rounded-full border border-primary/15 bg-white px-4 py-2.5 text-sm font-semibold text-text transition-colors hover:border-primary/30 hover:text-primary">{item}</button>)}</div></section>
