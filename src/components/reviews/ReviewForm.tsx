...
export function ReviewForm({ onSuccess }: ReviewFormProps) {
  ...
  const fieldClass = (error?: string) => `w-full h-11 rounded-xl border bg-white px-4 text-sm text-text focus:border-primary/60 focus:outline-none focus:ring-2 focus:ring-primary/10 transition-colors ${error ? 'border-red-400' : 'border-border'}`;
  return <div className="rounded-2xl bg-white border border-border shadow-soft p-6 md:p-8">
