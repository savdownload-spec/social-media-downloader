...
export function TrustStrip() {
  return (
    <Container className="pb-4">
      <div className="mx-auto max-w-5xl rounded-2xl border border-border bg-white/70 dark:bg-card/70 p-2.5 shadow-soft backdrop-blur md:p-3">
        ...
        <dl className="grid grid-cols-1 gap-px overflow-hidden rounded-xl bg-border-light sm:grid-cols-2 lg:grid-cols-4">
          {trustPoints.map((point, i) => {
            const Icon = icons[i];
            return (
              <div key={point.title} className="bg-white dark:bg-card p-5">
