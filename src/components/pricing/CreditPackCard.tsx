...
import { WAITLIST_CTA, type CreditPack } from '@/config/pricing';

/**
 * A one-time credit pack. Leads with the credit count, then price, then the
 * derived per-1,000 rate and a savings badge computed from the base pack.
 */
export function CreditPackCard({ pack }: { pack: CreditPack }) {
  const highlighted = pack.highlight !== 'none';
  ...
  return (
    <HighlightFrame variant={pack.highlight} badge={pack.badge} className="p-6">
      <div className="flex h-full flex-col">
        ...
        <div className="mt-auto">
          <Link href={WAITLIST_CTA.href} className={cta}>
            {WAITLIST_CTA.label}
          </Link>
        </div>
      </div>
