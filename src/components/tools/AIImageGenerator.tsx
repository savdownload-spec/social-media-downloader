...
import { AlertCircle, Download, Loader2, RefreshCw, Sparkles, WandSparkles } from 'lucide-react';
...
export function AIImageGenerator() {
  ...
  async function generate() {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 402) setStatus('insufficient_credits');
