...
function CopyRow({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={copy}
      className="w-full flex items-center justify-between gap-3 px-4 py-3 rounded-xl bg-surface hover:bg-primary-light border border-border transition-colors text-left"
    >
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full h-11 rounded-xl border border-border bg-white dark:bg-card px-4 text-sm text-text placeholder:text-text-subtle focus:border-primary/40 focus:outline-none transition-colors"
      />
    </div>
  );
}
  ...
  if (slug === 'meta-title-generator') {
    return (
      <div className="w-full max-w-2xl mx-auto space-y-6">
        <div className="p-6 rounded-2xl border border-border bg-white dark:bg-card shadow-soft space-y-4">
        ...
        {tags.length > 0 && (
          <div className="p-6 rounded-2xl border border-border bg-white dark:bg-card shadow-soft">
            <div className="flex flex-wrap gap-2 mb-4">
              {tags.map((t, i) => (
                <span key={i} className="px-3 py-1.5 rounded-full bg-primary-light text-xs font-medium text-text">{t}</span>
              ))}
            </div>
            <button
              onClick={() => { navigator.clipboard.writeText(tags.join(', ')); success('Copied!', 'All tags copied to clipboard.'); }}
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-border bg-white dark:bg-card text-sm font-semibold text-text hover:border-primary/40 transition-colors"
            >
        ...
        {keywordGroups.length > 0 && (
          <div className="space-y-4">
            {keywordGroups.map((group) => (
              <div key={group.intent} className="p-5 rounded-2xl border border-border bg-white dark:bg-card shadow-soft">
        ...
        <div className="p-6 rounded-2xl border border-border bg-white dark:bg-card shadow-soft space-y-4">
          <div>
            ...
            <select
              ...
              className="w-full h-11 rounded-xl border border-border bg-white dark:bg-card px-4 text-sm text-text focus:border-primary/40 focus:outline-none transition-colors"
            >
