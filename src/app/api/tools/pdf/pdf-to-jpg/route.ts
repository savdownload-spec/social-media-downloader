...
export async function POST(req: Request) {
  ...
  try {
    ...
    const limitViolation = await checkBatchLimit('pdf-to-jpg', files.length);
    ...
    const maxPages = Math.min(50, Math.max(1, Number(body.maxPages || 10)));
    const results = [];
    for (let index = 0; index < input.buffers.length; index += 1) {
      const result = await pdfToImages(input.buffers[index]!, maxPages, input.names[index]);
      if (!result.ok) { results.push({ sourceName: input.names[index], status: 'failed', error: result.error }); continue; }
      if (!(await gate.spend(`PDF to JPG: ${input.names[index]}`))) { results.push({ sourceName: input.names[index], status: 'failed', error: 'Credit balance changed before this file could be charged. Please retry.' }); continue; }
      results.push({ sourceName: input.names[index], status: 'completed', outputs: result.buffers.map((output) => ({ name: output.name, group: input.names[index], size: output.buffer.length, base64: Buffer.from(output.buffer).toString('base64') })) });
    }
  ...
  } catch { return NextResponse.json({ error: 'Could not render this PDF batch. Some files may be encrypted or damaged.' }, { status: 422 }); }
