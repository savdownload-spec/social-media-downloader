...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!files.length) return NextResponse.json({ error: 'Choose at least one Word document.' }, { status: 400 });
    const limitViolation = await checkBatchLimit('word-to-pdf', files.length);
    if (limitViolation) return limitViolation;
    if (files.some((file) => !/\.(doc|docx)$/i.test(file.name))) return NextResponse.json({ error: 'Word to PDF accepts DOC and DOCX files only.' }, { status: 400 });
