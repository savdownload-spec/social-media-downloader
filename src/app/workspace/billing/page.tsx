...
export default async function WorkspaceBillingPage({
  ...
  if (!session?.user?.id) redirect('/login?callbackUrl=/workspace/billing');
  ...
  if (!summary) redirect('/login?callbackUrl=/workspace/billing');
    ...
    <div className="px-4 sm:px-6 lg:px-10 py-8 md:py-10 max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-accent/20 bg-accent-light p-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-text-muted">
              Credits usually appear within a few seconds. Refresh this page if the balance below still looks
              unchanged.
            </p>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-border bg-white dark:bg-card p-6 shadow-soft">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Current plan</p>
        ...
        {subscription && (
          <p className="mt-4 border-t border-border-light pt-4 text-sm text-text-muted">
            {subscription.cancelAtPeriodEnd ? 'Your plan is set to cancel' : 'Renews'}{' '}
            {subscription.currentPeriodEnd ? `on ${subscription.currentPeriodEnd.toLocaleDateString()}` : 'at the end of the current period'}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <BalanceTile label="Total credits" value={summary.totalCredits} emphasis note="Everything available to spend" />
        <BalanceTile
          label="Plan allowance"
          value={summary.planCredits}
          note={summary.planCreditsResetAt ? `Resets ${summary.planCreditsResetAt.toLocaleDateString()}` : 'Refills each period'}
        />
        <BalanceTile label="Purchased" value={summary.purchasedCredits} note="Never expires" />
      </div>
          ...
          <DataSection>
            {ledger.map((entry) => (
              <div key={entry.id} className="flex items-center justify-between gap-4 px-4 py-3.5 border-b border-border-light last:border-0">
                ...
                <span className={`flex-shrink-0 text-sm font-semibold ${entry.amount >= 0 ? 'text-accent-hover' : 'text-text-muted'}`}>
                  {entry.amount >= 0 ? '+' : ''}
...
function BalanceTile({
  ...
  return (
    <div className={`rounded-2xl border p-5 ${emphasis ? 'border-primary/20 bg-primary-light' : 'border-border bg-white dark:bg-card'}`}>
      ...
      <p className="mt-2 font-display text-2xl font-bold tracking-tight text-text">{value.toLocaleString()}</p>
